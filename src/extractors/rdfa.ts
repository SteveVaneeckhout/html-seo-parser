...
interface Context {
  ...
  pendingProps: Record<string, StructuredDataValue[]> | null;
}

export function extractRdfa($: CheerioAPI): StructuredDataItem[] {
  const items: StructuredDataItem[] = [];
  const ctx: Context = { vocab: null, prefixes: {}, pendingProps: null };
...
function walk($: CheerioAPI, el: Element, ctx: Context, items: StructuredDataItem[]): void {
  const $el = $(el);
  ...
  const nextPrefixes =
    prefixAttr !== undefined ? mergePrefixes(ctx.prefixes, prefixAttr) : ctx.prefixes;

  let newItem: StructuredDataItem | null = null;
  let newPendingProps: Record<string, StructuredDataValue[]> | null = null;

  if (typeofAttr !== undefined) {
    newItem = {};
    newPendingProps = {};
    const types = splitTokens(typeofAttr).map((t) => resolveTerm(t, nextVocab, nextPrefixes));
    if (types.length === 1) newItem["@type"] = types[0]!;
    else if (types.length > 1) newItem["@type"] = types;

    const id = $el.attr("resource") ?? $el.attr("href") ?? $el.attr("src");
    if (id !== undefined && id.length > 0) newItem["@id"] = id;
  ...
  for (const child of el.children) {
    if (isElement(child)) walk($, child, childCtx, items);
  }
...
function extractPropertyValue($: CheerioAPI, el: Element): string {
  const $el = $(el);
...
function resolveTerm(term: string, vocab: string | null, prefixes: Record<string, string>): string {
  if (/^https?:\/\//i.test(term) || term.startsWith("urn:")) return term;
  const colonIdx = term.indexOf(":");
  if (colonIdx > 0) {
    const prefix = term.slice(0, colonIdx);
    const local = term.slice(colonIdx + 1);
    const expansion = prefixes[prefix];
    if (expansion !== undefined) return expansion + local;
    return term;
  }
  if (vocab !== null) return vocab + term;
  return term;
}

function mergePrefixes(base: Record<string, string>, prefixAttr: string): Record<string, string> {
